...
describe("Environment support integration tests", () => {
  ...
  beforeEach(() => {
    ...
    testDir = join(
      tmpdir(),
      `psst-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  const runPsst = async (args: string[], env: Record<string, string> = {}) => {
    const proc = Bun.spawn(
      ["bun", "run", join(originalCwd, "src/main.ts"), ...args],
      {
        cwd: testDir,
  ...
  describe("init with --env", () => {
    it("creates local vault with default env", async () => {
      ...
      expect(
        existsSync(join(testDir, ".psst", "envs", "default", "vault.db")),
      ).toBe(true);
    });
    ...
    it("creates local vault with custom env", async () => {
      ...
      expect(
        existsSync(join(testDir, ".psst", "envs", "prod", "vault.db")),
      ).toBe(true);
    });
    ...
    it("creates multiple environments", async () => {
      ...
      expect(
        existsSync(join(testDir, ".psst", "envs", "dev", "vault.db")),
      ).toBe(true);
      expect(
        existsSync(join(testDir, ".psst", "envs", "staging", "vault.db")),
      ).toBe(true);
      expect(
        existsSync(join(testDir, ".psst", "envs", "prod", "vault.db")),
      ).toBe(true);
  ...
  describe("PSST_ENV environment variable", () => {
    it("uses PSST_ENV when --env not specified", async () => {
      ...
      // List using PSST_ENV
      const listResult = await runPsst(["list", "--json"], {
        PSST_ENV: "staging",
      });
    ...
    it("--env flag overrides PSST_ENV", async () => {
      ...
      const result = await runPsst(["--env", "dev", "list", "--json"], {
        PSST_ENV: "prod",
      });
      const json = JSON.parse(result.stdout);
      expect(json.secrets).toHaveLength(1);
  ...
  describe("lock/unlock preserves secrets", () => {
    ...
    it("secrets survive lock/unlock cycle", async () => {
      ...
      expect(
        existsSync(
          join(testDir, ".psst", "envs", "default", "vault.db.locked"),
        ),
      ).toBe(true);
    ...
    it("fails with wrong unlock password", async () => {
      ...
      const unlockProc = Bun.spawn(
        ["bun", "run", join(originalCwd, "src/main.ts"), "unlock"],
        {
          cwd: testDir,
